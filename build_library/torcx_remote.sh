# Copyright (c) 2018 The Container Linux by CoreOS Authors. All rights
# reserved.
# Use of this source code is governed by a BSD-style license that can be
# found in the LICENSE file.

# torcx_remote.sh contains helper functions for creating, editing, and
# reading torcx manifest files.

# create_empty creates an empty torcx manfiest at the given path.
function torcx_remote::create_empty() {
  local path="${1}"
  jq '.' > "${path}" <<EOF
{
...
#
# Example:
#     pkg_name_arr=($(torcx_remote::get_pkg_names "torcx_manifest.json"))
function torcx_remote::get_pkg_names() {
  local file="${1}"
...
function torcx_remote::local_store_path() {
  local file="${1}"
  local name="${2}"
  local version="${3}"
...
function torcx_remote::get_digest() {
  local file="${1}"
  local name="${2}"
  local version="${3}"
...
function torcx_remote::get_digests() {
  local file="${1}"
  local name="${2}"
...
function torcx_remote::get_versions() {
  local file="${1}"
  local name="${2}"
...
function torcx_remote::default_version() {
  local file="${1}"
  local name="${2}"
...
function torcx_remote::sources_on_disk() {
  local file="${1}"
